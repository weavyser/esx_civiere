...
RegisterCommand('civiere', function()
	...
	--local civiere = CreateObject(GetHashKey('v_med_bed1'), GetEntityCoords(PlayerPedId()), true)
end, false)

RegisterCommand('removeciviere', function()
	local civiere = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 10.0, GetHashKey('v_med_bed1'))
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 2.0 then
				DrawText3Ds(sitCoords, "[E] Se coucher", 0.4)
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 2.0 then
				DrawText3Ds(pickupCoords, "[E] Poussez le lit", 0.4)
